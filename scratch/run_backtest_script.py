...
def main():
    ...
    eurusd = cache["eurusd"]
    # We want to run the backtest starting from 2021-09-01 through the end of the eurusd dataset
    # Filter dates (convert Timestamp to string)
    ...
    # We run the backtest. To be fast, we'll iterate through the dates
    # Since we have cached everything, it should be relatively quick.
    count = 0
    total_days = len(test_dates)
    
    # We'll use H4 data if it is available and date is >= H4 start date (approx 2023-07-10)
    # Check if H4 data exists
    ...
    # We look up the future prices in the main eurusd daily dataset
    # Sort eurusd by date
    ...
    # Add forward returns and drawdowns
    forward_returns_1 = []
    forward_returns_3 = []
    forward_returns_5 = []
    forward_returns_10 = []
    ...
    for i, row in df_res.iterrows():
        ...
        # 10-day max drawdown (look forward up to 10 days)
        # For long: min(low_k - close_t) / close_t
        # For short: max(high_k - close_t) / close_t
        # Let's collect low and high for next 10 days
        ...
        if next_10_dates:
            ...
            if "strong bullish" in verdict or "bullish" in verdict:
                ...
                # Short position drawdown (price goes up)
    ...
    # Calculate daily trade return for Sharpe (using 1d return)
    # Long if Bullish, Short if Bearish, 0 if Neutral
    trade_returns = []
    for i, row in df_res.iterrows():
        v = row["verdict"].lower()
        r = row["ret_1d"]
    ...
    # Historical Volatility (20-day standard deviation of daily close returns)
    ...
    for cat in categories:
        ...
        if "bullish" in cat.lower():
            ...
            # For Neutral, report percentage of positive days
            win_1 = (df_cat["ret_1d"] > 0).sum() / df_cat["ret_1d"].dropna().count() if df_cat["ret_1d"].dropna().count() > 0 else np.nan
            win_3 = (df_cat["ret_3d"] > 0).sum() / df_cat["ret_3d"].dropna().count() if df_cat["ret_3d"].dropna().count() > 0 else np.nan
            win_5 = (df_cat["ret_5d"] > 0).sum() / df_cat["ret_5d"].dropna().count() if df_cat["ret_5d"].dropna().count() > 0 else np.nan
            win_10 = (df_cat["ret_10d"] > 0).sum() / df_cat["ret_10d"].dropna().count() if df_cat["ret_10d"].dropna().count() > 0 else np.nan
    ...
    # Active Signals = Bullish/Strong Bullish and Bearish/Strong Bearish
    active_signals = df_res[df_res["verdict"].str.lower().str.contains("bullish|bearish")]
    
    # 5-day Accuracy and expectancy
    ...
    # 10-day Accuracy and expectancy
    ...
    # Annualized Sharpe Ratio of daily trade returns
